...
export type PetState = 'sleep' | 'idle' | 'dance' | 'jump' | 'headbang' | 'fire'
...
export class PetEngine {
  ...
  update(energy: BeatEnergy, dt: number): void {
    ...
    // Track bass sustain for fire state
    if (energy.bass > 0.8 && energy.overall > 0.6) {
  ...
  private renderFrog(ctx: CanvasRenderingContext2D): void {
    ...
    else if (s === 'fire') this.crabFire(ctx)
  ...
  private crabFire(ctx: CanvasRenderingContext2D): void {
    const { body, dark } = this.colors
    const outline = shade(body, 70)
    const light = tint(body, 50)
    const shake = Math.round(Math.sin(this.time * 15) * 1)
    ...
    // Body
    for (let x = -5; x <= 5; x++) for (let y = -1; y <= 4; y++) this.d(ctx, x, y, body)
    for (let x = -6; x <= 6; x++) for (let y = 1; y <= 3; y++) this.d(ctx, x, y, body)
    for (let x = -4; x <= 4; x++) this.d(ctx, x, -2, body)
    for (let x = -3; x <= 3; x++) this.d(ctx, x, -3, body)
    ...
    this.d(ctx, -8, -2, body)
    this.d(ctx, -9, -3, body)
    this.d(ctx, -10, -4, body)
    // Left claw open
    this.d(ctx, -10, -5, outline)
    this.d(ctx, -11, -6, outline)
    this.d(ctx, -8, -5, outline)
    this.d(ctx, -7, -6, outline)
    ...
    // Fire on left claw
    if (firePhase === 0) {
      this.d(ctx, -10, -7, fireColors[0])
      this.d(ctx, -9, -7, fireColors[1])
      this.d(ctx, -10, -8, fireColors[2])
    } else if (firePhase === 1) {
      this.d(ctx, -10, -7, fireColors[1])
      this.d(ctx, -9, -7, fireColors[2])
      this.d(ctx, -10, -8, fireColors[3])
      this.d(ctx, -9, -8, fireColors[0])
    } else if (firePhase === 2) {
      this.d(ctx, -10, -7, fireColors[2])
      this.d(ctx, -9, -7, fireColors[3])
      this.d(ctx, -10, -8, fireColors[0])
      this.d(ctx, -9, -8, fireColors[1])
      this.d(ctx, -10, -9, fireColors[3])
    } else {
      this.d(ctx, -10, -7, fireColors[3])
      this.d(ctx, -9, -7, fireColors[0])
      this.d(ctx, -10, -8, fireColors[1])
      this.d(ctx, -9, -8, fireColors[2])
    }
